					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								wsprintf(nameStr, L"Autismgram Desktop");
								...
								wsprintf(icongroupStr, L"Autismgram Desktop");
...
static const WCHAR *_programName = L"Autismgram Desktop"; // folder in APPDATA, if current path is unavailable for writing
